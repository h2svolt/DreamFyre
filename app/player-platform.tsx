...
function Explore({ data, action, loading, go }: { data: Snapshot; action: (p: Record<string, unknown>) => Promise<unknown>; loading: boolean; go: (v: View) => void }) {
  ...
  return <div className="stack"><PageHead title="Rewards lounge" text="Daily rewards, weekly missions, VIP levels and FreePlay exchange—all in one place." action={<button className="secondary" onClick={() => go("referrals")}><Gift />Invite players</button>}/>
