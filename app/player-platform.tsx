...
export function PlayerPlatform({ initialAuthenticated = false }: { initialAuthenticated?: boolean }) {
  ...
  const content = {
    ...
    profile: <Profile data={data} installApp={installApp} go={setView} logout={logout} />,
  }[view];
...
function Profile({ data, installApp, go, logout }: { data: Snapshot; installApp: () => void; go: (v: View) => void; logout: () => Promise<void> }) {
  return <AccountCenter player={data.user} installApp={installApp} go={go} logout={logout}/>;
}
